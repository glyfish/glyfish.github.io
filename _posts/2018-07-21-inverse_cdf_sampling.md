...
values of {% katex %}n{% endkatex %} will have the distribution {% katex %}(2){% endkatex %}.

The figure below favorably compares generated samples and {% katex %}(2){% endkatex %},
...
moments of the distribution computed from the samples with the target value computed analytically.
For the distribution {% katex %}(2){% endkatex %}. In general for a discrete distribution the first and second moments
are given by,
...
{% katex %}\sigma{% endkatex %} computed from the samples generated by distribution
{% katex %}(2){% endkatex %} using Inverse CDF Sampling are compared with the target values using the equations above.
...
target value and by {% katex %}10^4{% endkatex %} the values computed from samples and the target
...
are generated. The desired samples with the Weibull distribution are then computed using the CDF inverse.
...
computed from Inverse CDF Sampling of distribution {% katex %}(5){% endkatex %} with the target values from the equations above.
...
samples are comparable to the target values and by {% katex %}10^4{% endkatex %} the values computed from samples and the target
values are indistinguishable.
