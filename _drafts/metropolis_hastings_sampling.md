...
at the previous time step, namely,
...
probability. Recall a proposal sample, {% katex %}y^{\ast}{% endkatex %}, generated by
{% katex %}q(x,y){% endkatex %}, independent of {% katex %}U{% endkatex %} is accepted if,

{% katex display %}
U\ \leq\ \alpha(x, y^{\ast}),
{% endkatex %}

then {% katex %}y=y^{\ast}{% endkatex %} and rejected if,
...
probability that it is rejected.

Consider the first term where {% katex %}U\ \leq\ \alpha(x, y^{\ast}){% endkatex %} and
{% katex %}y=y^{\ast}{% endkatex %},

{% katex display %}
P\left[Y\ \leq\ y,\ \leq\ \alpha(x, y^{\ast})\ \mid\ X=x \right] = P\left[Y\ \leq\ y\ \mid\ U\ \leq\ \alpha(x, y^{\ast}),\ X=x \right]P\left[U\ \leq\ \alpha(x, y^{\ast})\ \mid\ X=x \right],
{% endkatex %}
