...
{% katex %}0\ \leq\ \frac{f_X(Y)}{cf_Y(Y)}\ \leq\ 1, \ \ \forall\ Y{% endkatex %}. If equation
...
| :-----: | :---- |
| `h` | A function of the target and proposal distributions, {% katex %}f_X(Y){% endkatex %} and {% katex %}f_Y(Y){% endkatex %} respectively, defined by {% katex %}h(Y)=f_X(Y)/f_Y(Y){% endkatex %}.|
...
The following sections will compare the performance of generating Weibull samples using {% katex %}\textbf{Uniform}(0, 1){% endkatex %} and
