...
The above function `rejection_sample(h, y_samples, c)` takes three arguments as input. `h` is the ratio of the target distribution
{% katex %}f_X(Y){% endkatex %} and the proposal distribution {% katex %}f_Y(Y){% endkatex %}. `y_samples` is an array of samples generated by
the proposal distribution {% katex %}f_Y(y){% endkatex %} and `c` is a constant chosen so that {% katex %}0\ \leq\ h(Y)/c\ \leq\ 1{% endkatex %}.
More information is about the parameters is given in the table below.
...
{% katex %}\textbf{Uniform}(0, m){% endkatex %} proposal distribution requires that a bound be placed
...
distribution and target distribution {% katex %}(4){% endkatex %} is discussed.
A {% katex %}\textbf{Normal}(/mu, /sigma){% endkatex %} proposal distribution has advantages over
the {% katex %}\textbf{Uniform}(0, m){% endkatex %} proposal distribution discussed in he previous
section. First, it can provide unbounded samples, while a uniform proposal requires specifying bounds
on the samples. Second, it is a closer approximation to the target distribution so can provide samples
that are accepted with greater frequency.

A disadvantage of the {% katex %}\textbf{Normal}(/mu, /sigma){% endkatex %} proposal distribution is that it requires specification of {% katex %}\mu{% endkatex %} and
{% katex %}\sigma{% endkatex %}. If these parameters are the slightest off the performance of the
...
{% katex %}h(y){% endkatex %}. There is a large peak in {% katex %}h(y){% endkatex %} to right cause
